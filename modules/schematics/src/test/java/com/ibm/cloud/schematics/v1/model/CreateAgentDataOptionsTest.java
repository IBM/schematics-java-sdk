...
/**
 * Unit test class for the CreateAgentDataOptions model.
 */
public class CreateAgentDataOptionsTest {
  final HashMap<String, InputStream> mockStreamMap = TestUtilities.createMockStreamMap();
  final List<FileWithMetadata> mockListFileWithMetadata = TestUtilities.creatMockListFileWithMetadata();

  @Test
  public void testCreateAgentDataOptions() throws Throwable {
    ...
    CreateAgentDataOptions createAgentDataOptionsModel = new CreateAgentDataOptions.Builder()
      ...
      .build();
    assertEquals(createAgentDataOptionsModel.name(), "MyDevAgent");
    assertEquals(createAgentDataOptionsModel.resourceGroup(), "Default");
    assertEquals(createAgentDataOptionsModel.version(), "v1.0.0");
    assertEquals(createAgentDataOptionsModel.schematicsLocation(), "us-south");
    assertEquals(createAgentDataOptionsModel.agentLocation(), "us-south");
    assertEquals(createAgentDataOptionsModel.agentInfrastructure(), agentInfrastructureModel);
    assertEquals(createAgentDataOptionsModel.description(), "Create Agent");
    assertEquals(createAgentDataOptionsModel.tags(), java.util.Arrays.asList("testString"));
    assertEquals(createAgentDataOptionsModel.agentMetadata(), java.util.Arrays.asList(agentMetadataInfoModel));
    assertEquals(createAgentDataOptionsModel.agentInputs(), java.util.Arrays.asList(variableDataModel));
    assertEquals(createAgentDataOptionsModel.userState(), agentUserStateModel);
    assertEquals(createAgentDataOptionsModel.agentKpi(), agentKpiDataModel);
  }

  @Test(expectedExceptions = IllegalArgumentException.class)
  public void testCreateAgentDataOptionsError() throws Throwable {
    new CreateAgentDataOptions.Builder().build();
  }
